...
if test x"$WITH_FOX_1_0" != xno; then
	if test x"$have_FOX" != xyes; then
		CHECK_LIBFOX__()
		if test x"$have_FOX" = xyes; then
...
AM_CONDITIONAL(HAVE_FOX_1_0, test x"$have_FOX_1_2" != xyes -a x"$have_FOX" = xyes)
...
dnl
dnl @version $Id: aclocal.m4,v 1.73 2004/09/05 11:04:26 pini Exp $
